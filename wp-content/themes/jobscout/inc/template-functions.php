...
if (!function_exists('jobscout_content_start')):
    ...
    function jobscout_content_start()
    {
        ...
        if (!(is_front_page() && !is_home() && $home_sections)) { //Make necessary adjust for pg template.
            echo is_404() ? '<div class="error-holder">' : '<div id="content" class="site-content bg-for-task-5">';
            ?>
                <?php
...
if (!function_exists('jobscout_entry_header')) :
    ...
    { ?>
        <header class="entry-header">
            ...
            if (is_singular()):
                echo '<h1 class="entry-title">' . esc_html($short_title) . '</h1>';
            else:
                echo '<h2 class="entry-title"><a href="' . esc_url(get_permalink()) . '" rel="bookmark">' . esc_html($short_title) . '</a></h2>';
            endif;
            ?>
        </header>
    <?php
...
if (!function_exists('jobscout_entry_content')):
    ...
    function jobscout_entry_content()
    {
        $ed_excerpt = get_theme_mod('ed_excerpt', true); ?>
                <div class="entry-content enyan27" itemprop="text">
                    <?php
...
// echo '<a href="' . esc_url( get_the_permalink() ) . '" class="readmore-link"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16.207 8.58"><defs><style>.c{fill:none;stroke:#2ace5e;}</styl></defs><g transform="translate(-701.5 -958.173)"><path class="c" d="M-9326.909-9204.917l-3.937,3.937,3.937,3.937" transform="translate(-8613.846 -8238.518) rotate(180)"/><line class="c" x2="15.154" transform="translate(701.5 962.426)"/></g></svg>' . esc_html( $readmore ) . '</a>';    
            
                        // Task-5 - Read more
                        echo '<a href="' . esc_url(get_the_permalink()) . '" class="readmore-link" style="color:#ff9068";>' . esc_html($readmore) . '</a>';
                    }
